...
if ($conn->connect_error) {
    file_put_contents('error_log.txt', "Connection failed: " . $conn->connect_error . "\n", FILE_APPEND);
    exit;
}
    ...
    foreach ($data['results'] as $record) {
        ...
        if ($result_check->num_rows > 0) {
            ...
            if ($conn->query($sql_update) === TRUE) {
                ...
                file_put_contents('error_log.txt', "Error updating record: " . $conn->error . "\n", FILE_APPEND);
            }
        } else {
            // Insert a new record
            ...
            if ($conn->query($sql_insert) === TRUE) {
                ...
                file_put_contents('error_log.txt', "Error inserting new record: " . $conn->error . "\n", FILE_APPEND);
            }
        }
    }
} else {
    file_put_contents('error_log.txt', "Failed to retrieve data from KoboToolbox. HTTP Code: " . $http_code . "\n", FILE_APPEND); 
}
